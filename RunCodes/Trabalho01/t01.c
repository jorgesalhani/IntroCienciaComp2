...
bool position_is_available(char*** pointer_to_mine_map, int i, int j) {
    char** mine_map = *pointer_to_mine_map;
    if (mine_map[i][j] == '.') {
        return true;
    }
...
bool cursor_has_available_near_position(char*** pointer_to_mine_map, int* cursor_x, int* cursor_y, int* M, int* N) {
    int* edge_limits = NULL;
    edge_limits = calloc(4, sizeof(int));
...
bool position_has_breadcrumb(char*** pointer_to_mine_map, int i, int j) {
    char** mine_map = *pointer_to_mine_map;
    if (mine_map[i][j] == '0') {
        return true;
    }
...
bool breadcrumbs_fallback(char*** pointer_to_mine_map, int* cursor_x, int* cursor_y, int* M, int* N) {
    ...
    // for (int k = 0; k < 4; k++) printf("%d ", edge_limits[k]);
    // printf("\n");
    for (int x = edge_limits[1] - 1; x >= edge_limits[0]; x--) {
        for (int y = edge_limits[3] - 1; y >= edge_limits[2]; y--) {
            if (position_has_breadcrumb(pointer_to_mine_map, (i + x), (j + y))) {
                *cursor_x = i + x;
                *cursor_y = j + y;
                // printf("%d %d\n", *cursor_x, *cursor_y);
                free(edge_limits);
...
void partially_hide_map(char*** pointer_to_mine_map, int* cursor_x, int* cursor_y, int* M, int* N) {
    ...
    if (!cursor_has_available_near_position(pointer_to_mine_map, cursor_x, cursor_y, M, N)) {
        // return;
        if (!breadcrumbs_fallback(pointer_to_mine_map, cursor_x, cursor_y, M, N)) {
            return;
        }
    }
